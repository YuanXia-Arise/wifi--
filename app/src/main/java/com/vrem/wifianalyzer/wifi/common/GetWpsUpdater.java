...
public class GetWpsUpdater extends AsyncTask<Object, Object, List<WiFiDetail>> {
    Context mContext;
    ListView mListView;
    String mDevId;
    int mTag;
    ...
    public GetWpsUpdater(Context context, ListView listView, String devId, int tag, ProgressBar progressBar, int mainPage, int sort, TextView refresh, TextView noData, boolean isDialog) {
        mContext = context;
        mListView = listView;
        mDevId = devId;
        mTag = tag;
    ...
    protected List<WiFiDetail> doInBackground  (Object... params) {
        try {
            ...
            List<WiFiDetail> apData = WiFiDetail.responseWpsApData(response, mTag, mSort);
            if (apData == null) {
                mError = true;
            }
