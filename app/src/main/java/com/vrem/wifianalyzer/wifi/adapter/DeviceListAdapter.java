...
//			BackgroundTask.timerInfoStart(context);
			...
			switch (device.getWorkType()) {
				case 100:
					...
					listItemView.deviceLayout.setEnabled(true);
					listItemView.stopButton.setVisibility(View.VISIBLE);//设置隐藏按钮
...
//					devStatusDBUtils.close();
				...
				case 101:
					...
					try {
						...
						if (handlingDetail.length() > 17){
							handlingDetail = handlingDetail.substring(0,17);
							boo = true;
						}
						MacSsidDBUtils macSsidDBUtils = null;
						macSsidDBUtils = new MacSsidDBUtils(context);
						macSsidDBUtils.open();
						System.out.println("20218888==>" + device.getDevId() + handlingDetail);
						String Ssid = macSsidDBUtils.getSSID(device.getDevId(),handlingDetail);

						if (boo) {
							if (Ssid.contains("_5G")) {
								Ssid = Ssid.replace("_5G", "") + "(2.4G/5G)";
							} else if (Ssid.contains("-5G")) {
								Ssid = Ssid.replace("-5G", "") + "(2.4G/5G)";
							} else if (Ssid.contains(" 5G")) {
								Ssid = Ssid.replace(" 5G", "") + "(2.4G/5G)";
							} else {
								Ssid = Ssid + "(2.4G/5G)";
...
//							Log.d("数据：", String.valueOf(sb.append(strLine)));
					...
					break;
				case 102:
					listItemView.devStatus.setText("正在进行单信道定向阻断");
					listItemView.deviceLayout.setEnabled(true);
					listItemView.stopButton.setVisibility(View.VISIBLE);//设置按钮可见
					...
					try {
						String handlingDetail = getHandlingDetail(device.getDevId());
						moreView.setText("定向阻断信道为:" + handlingDetail);
				...
				case 103:
					...
					listItemView.expandableListView.setVisibility(View.GONE);
					// listItemView.stateView.setBackgroundColor(Color.parseColor("#EE0000"));
					listItemView.devIcon.setBackgroundResource(R.drawable.wifiblue);
				...
				case 105:
					...
					try{
						joo.put("net",PrefSingleton.getInstance().getString("fake_net"));
						joo.put("out",PrefSingleton.getInstance().getString("fake_out"));
						joo.put("essid",PrefSingleton.getInstance().getString("fake_essid"));
						joo.put("channel",PrefSingleton.getInstance().getInt("fake_channel"));
						joo.put("security",PrefSingleton.getInstance().getString("fake_security"));
						joo.put("password",PrefSingleton.getInstance().getString("fake_password"));
						joo.put("encryption",PrefSingleton.getInstance().getString("fake_encryption"));
						joo.put("ssid",PrefSingleton.getInstance().getString("fake_ssid"));
						joo.put("ticket",PrefSingleton.getInstance().getString("fake_ticket"));
						joo.put("ap_channel",PrefSingleton.getInstance().getInt("fake_ap_channel"));
					} catch (JSONException e) {
						e.printStackTrace();
					} catch (NullPointerException e){}
					final JSONObject joof = joo;
					...
					BackgroundTask.mTimerTaskHandling = new TimerTask() {
						...
						public void run() {
							((Activity) context).runOnUiThread(new Runnable() {
								@Override
								public void run() {
									new FakeAPUpdater(context, device.getDevId(), joof).execute();
								}
							});
						}
					};
				...
				case 201:
					...
					try{
						jo.put("net",PrefSingleton.getInstance().getString("fake_net"));
						jo.put("out",PrefSingleton.getInstance().getString("fake_out"));
						jo.put("essid",PrefSingleton.getInstance().getString("fake_essid"));
						jo.put("channel",PrefSingleton.getInstance().getInt("fake_channel"));
						jo.put("security",PrefSingleton.getInstance().getString("fake_security"));
						jo.put("password",PrefSingleton.getInstance().getString("fake_password"));
						jo.put("encryption",PrefSingleton.getInstance().getString("fake_encryption"));
						jo.put("ssid",PrefSingleton.getInstance().getString("fake_ssid"));
						jo.put("ticket",PrefSingleton.getInstance().getString("fake_ticket"));
						jo.put("ap_channel",PrefSingleton.getInstance().getInt("fake_ap_channel"));
					} catch (JSONException e) {
						e.printStackTrace();
					} catch (NullPointerException e){}
					final JSONObject jof = jo;
					System.out.println("20200911热点伪造0<==>" + jo);
					...
					BackgroundTask.mTimerTaskHandling = new TimerTask() {
						...
						public void run() {
							((Activity) context).runOnUiThread(new Runnable() {
								@Override
								public void run() {
									new FakeAPUpdater(context, device.getDevId(), jof).execute();
								}
							});
						}
					};
					...
					break;
				case 110:
					listItemView.devStatus.setText("正在进行多信道定向阻断");
					listItemView.deviceLayout.setEnabled(true);
					listItemView.stopButton.setVisibility(View.VISIBLE);
					// listItemView.stateView.setBackgroundColor(Color.parseColor("#EE0000"));
					listItemView.devIcon.setBackgroundResource(R.drawable.wifiblue);
					try {
						moreView.setText("定向阻断信道为:" + getHandlingDetail(device.getDevId()));
					} catch (/*JSON*/Exception e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
				...
				listView.setOnItemClickListener(new OnItemClickListener() {
					...
					public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
						...
						switch (arg2) {
							...
							// PackageInfo packageInfo = (PackageInfo) ssid.getTag();
							case 0: //设备重命名 （暂无法进行设置，设置不生效）
								dialog.dismiss();
						...
						new DialogInterface.OnClickListener() {
							...
							public void onClick(DialogInterface arg0, int arg1) {
								...
								//发送结束命令"cancelall"
										...
										try {
											param.put("action", "cancelall"); // 发送结束命令
											obj.put("param", param);
											System.out.println("20200909:" + obj);
										} catch (JSONException e) {
											e.printStackTrace();
										}
